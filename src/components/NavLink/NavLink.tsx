...
import { flatironTheme, Media, Theme } from '~/styles'
...
const NavLinkRenderFn: TNavLinkRenderFn = ({ children, icon, theme, ...props }, ref) => {
  const SelectedIcon = Icon[icon]

  return (
    <StyledNavLink ref={ref} theme={theme ?? flatironTheme} {...props}>
      <SelectedIcon />
      <span>{children}</span>
    </StyledNavLink>
  )
